...
AudioFormat = Literal["wav", "mp3", "flac", "m4a", "ogg", "webm", "aac"]
DownloadAudioFormat = Literal["wav", "mp3"]
...
PYDUB_FORMAT_BY_FORMAT: Mapping[AudioFormat, str] = {
    "wav": "wav",
    "mp3": "mp3",
    "flac": "flac",
    "m4a": "m4a",
    ...
    *,
    ...
    try:
        ...
        segment = audio_segment_cls.from_file(
            BytesIO(data),
            format=PYDUB_FORMAT_BY_FORMAT[source_format],
        )
        output = BytesIO()
        segment.export(output, format=PYDUB_FORMAT_BY_FORMAT[target_format])
