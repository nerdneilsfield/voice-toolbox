...
def test_fish_provider_has_no_clone_capability(tmp_path: Path) -> None:
    provider = FishAudioProvider(
        config=make_default_fish_audio_provider_config(),
        api_key="secret",
        artifact_root=tmp_path,
    ...
    assert provider.capabilities() == {"tts.builtin", "tts.design", "asr.transcribe"}
    assert "tts.clone" not in provider.capabilities()
    with pytest.raises(UnsupportedCapability):
        provider._validate_tts_request(
            TTSRequest(
                provider_id="fish-audio",
                mode=TTSMode.CLONE,
                text="hello",
                clone_sample_path=tmp_path / "sample.wav",
